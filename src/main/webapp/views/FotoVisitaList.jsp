...
<body>
	...
	<a href="adicionarFotos?id=${visitaId}" class="btn btn-primary" style="margin-left: 20px;" role="button">Adicionar mais fotos</a>
	<br />
	<br />
	<div style="margin-left: 20px; margin-right: 20px;">
		<table id="lista"
			...
			<c:forEach items="${listFotos}" var="foto" varStatus="status">
				<tr class="clickableRow" href="alteraFoto?id=${foto.id}">
					<td>${status.index + 1}</td>
					<td>${foto.visita.escola.nome}</td>
					<td><fmt:formatDate pattern="dd-MM-yyyy" value="${foto.visita.data}" /></td>
					<td>${foto.nome}</td>
					<td>
					...
					<td><a href="alteraFoto?id=${foto.id}">Alterar</a></td>
					<td><a href="deletaFotoVisita?id=${foto.id}&visitaId=${foto.visita.id}">Deletar</a></td>
				</tr>
			</c:forEach>
		</table>
	</div>
